...
const store = () => new Vuex.Store({
  ...
  mutations: {
    init(state){
      let dataJson = localStorage.getItem(config.app.name+"-data");
      if(dataJson===null){
    ...
    updateMember(state, data){
      ...
      localStorage.setItem(config.app.name+"-data",JSON.stringify(localData));
    },
    rmToken(state){
      rmToken(state);
    },
...
function rmToken(state) {
  state.member=undefined;state.token=undefined;
  localStorage.removeItem(config.app.name+"-data");
}
