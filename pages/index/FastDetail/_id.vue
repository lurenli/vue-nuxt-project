  ...
  <div class="IdDetail">
    ...
    <section id="main" class="clear">
      ...
      <!--<div class="bread-crumb">-->
        <!--<a href="/">首页</a>-->
        <!--<span>&gt;</span>-->
        <!--<a href="/LifeQuality/search">{{fenlei}}</a>-->
        <!--<span>&gt;</span>-->
          ...
          <h1>
            ...
            <div style="display:inline-block;width: 500px">
              <ul id="icon_list">
                ...
                <!--</li>-->
                <li v-for="(item,i) in summaryDates.pdpicture" :key="i" @click="choosePic(item)">
                  <img :src="$oss.publicUrl(item)" v-bind:style="listenPic(item,showPic)"
            ...
            <ul id="summary">
                ...
                <div>
                  <ul class="attrUI">
                    <li v-for="(item,sonIndex) in items.son"  :key="sonIndex" @click="clickattr(item.paramvalue,index,sonIndex)"
                        v-bind:class='item.show === 1 ? "checkedLi": item.show === 2 ? "disableLi":"normalLi"'>{{item.paramvalue}}<i></i></li>
                  </ul>
                </div>
              </li>
              <li>
              ...
              <li class="amount">
                ...
                <div>
                  ...
                  <span style="font-size: 12px" v-if="showprice">(库存{{showstroenum}}{{summaryDates.unit}})</span>
                </div>
              </li>
            </ul>
          </div>
    ...
    <div class="detail_down">
      <div class="right_tab">
        <ul class="tabs">
          <li v-for="(tabsData,index) in tabsDatas" :key="index" @click="toggle(index)" :class="{active:active==index}"
              style="cursor: pointer">
            {{tabsData}}
          </li>
        </ul>
        ...
        <div class="appraise" v-show="isShow2">
          <ul>
            ...
            <li class="pingjia" v-for="(listson,index) in evalutes" :key="index">
...
//        根据上一页的商品id,获取该商品的信息
      goodLoading: function () {
        ...
        vue.$http.post(
          ...
          function (vue, data) {
            // 成功处理
            vue.showstroenum = vue.Onemax(data.data.prodStoreList);
            vue.weights = vue.Onemin(data.data.prodStoreList);
...
//              处理远期价格的显示
          ...
          for (var w = 0; w < this.shopattr.length; w++) {
            ...
            this.shopattr[w].priceGroup = time1;
            this.timegroup = this.shopattr[w].priceGroup;
          }
          this.showstroenum = this.Onemax(this.shopattr);
          this.showprice = null;
...
<style scoped>
  ...
  #summary .attrUI  li i{
    ...
    right:-1px;
    bottom:-2px;
    background:url(../../../assets/i_selected.png) no-repeat right bottom;
    z-index:99;
    display:none;
  }
  /*正常状态*/
  #summary .attrUI  li.normalLi{
    cursor: pointer;
  }
  /*选中的样式*/
  #summary .attrUI  li.checkedLi {
    border:2px solid #e8000e;
    cursor: pointer;
  }
  #summary .attrUI  li.checkedLi i{
    display: block;
  }
  /*不可选择的样式*/
  #summary .attrUI  li.disableLi {
    color:#fff;
