<template>
  <div class="IdDetail">
                ...
                <div>
                  <ul class="attrUI">
                    <li v-for="(item,sonIndex) in items.son"  @click="clickattr(item.paramvalue,index,sonIndex)"
                        :class='item.show === 1 ? "checkedLi": item.show === 2 ? "disableLi":"normalLi"' :key="sonIndex"
                    >{{item.paramvalue}}<i></i>
                    </li>
                  </ul>
                </div>
    ...
    <div class="detail_down">
      <div class="right_tab">
        <ul class="tabs">
          <li v-for="(tabsData,index) in tabsDatas" :key="index" @click="toggle(index)" :class="{active:active==index}" style="cursor: pointer">
            {{tabsData}}
          </li>
        </ul>
