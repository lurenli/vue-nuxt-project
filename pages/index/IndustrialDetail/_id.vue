    ...
    <div class="IdDetail">
      ...
      <section id="main" class="clear">
        ...
        <div class="bread-crumb">
          <a href="/">首页</a>
          <span>&gt;</span>
          <a :href="'/IndustrialProducts/search-pageNo_1-pageSize_10-level1_'+getInformations.level1+'-selfsupport_0-havestore_0-forwardtime_0'">{{getInformations.level1}}</a>
          <span>&gt;</span>
          <a :href="'/IndustrialProducts/search-pageNo_1-pageSize_10-level2_'+getInformations.level2+'-selfsupport_0-havestore_0-forwardtime_0'">{{getInformations.level2}}</a>
          <span>&gt;</span>
          <span>{{getInformations.level3}}</span>
          <span>/</span>
          <span>{{getInformations.material}}</span>
          <span>/</span>
          <span>{{getInformations.mark}}</span>
          <span>/</span>
          <span>{{getInformations.stand}}</span>
          <span>/</span>
          <span>{{getInformations.surfacetreatment}}</span>
        </div>
        <!--产品简介-->
        <div id="product_intro">
          <div id="preview">
            <div class="smallpic" id="box">
              <img style="width: 350px;height: 350px" :src="$oss.publicUrl(showPic)"/>
              <div id="move"></div>
              <div id="bimg">
            ...
            <h1>
              ...
              <ul id="icon_list">
                <li v-for="(item,i) in getInformations.pdpicture" :key="i" @click="choosePic(item)">
            ...
            <ul id="summary">
              ...
              <ul>
                <li class="choose">
                  ...
                  <select @change="selectVal2(selecte2d.price)" v-model="selecte2d">
                    <option v-for="(item,i) in getInformations.prices" :key="i" :value="item">
                      {{ item.name }}
                    </option>
                  </select>
                </li>
              ...
              <li class="amount" style="position: relative">
                ...
                <select class="num_danwei" id="mySelect" @change="selectVal" v-model="selected">
                  <option v-for="(item,i) in selects" :key="i" v-bind:value="item.unit" class="danwei">
                    {{ item.unit}}
                  </option>
                </select>
                <ul class="money_price" style="position: absolute;left: 310px;top: -25px;width: 236px;">
                  <li class="wholemoney" v-for="(wholemoney,index) in wholemoneies" :key="index"
      ...
      <div class="detail_down">
        <div class="right_tab">
          <ul class="tabs">
            <li v-for="(tabsData,index) in tabsDatas" :key="index" @click="toggle(index)"
          ...
          <div class="appraise" v-show="isShow2">
            <ul>
              ...
              <li class="pingjia" v-for="(listson,i) in evalutes" :key="i">
...
//正在开发中
      ...
      inputFunc(val) {
        ...
        this.prodprice2 = singlePrice(this,intValue);
      },
      inputBlur(val) {
        if (this.checkNumber(val) < this.getInformations.productStore.startnum) {
          ...
          this.prodprice2 = singlePrice(this,this.getInformations.productStore.startnum);
          ...
          this.prodprice2 = singlePrice(this,this.getInformations.productStore.pdstorenum);
          ...
          this.getInformations.pagetypeStr = this.transPackagetype(this.getInformations.packingList,this.checkNumber(val),this.getInformations.unit);
        }
      },
        ...
        if(type&&num&&unit){
          ...
          if(type[0].num===0){
            ...
            if(num>0&&num<type[0].num){
              third = num.toFixed(3) + type[0].unit
...
//          console.log(pagetypeStr)
      ...
      selectVal: function (ele) {
        ...
        if (this.checkNumber(this.startnum) < this.getInformations.productStore.startnum) {
          ...
          this.startnum = parseInt(this.startnum);
          this.getInformations.pagetypeStr = transPackagetype(this.getInformations.packingList,this.checkNumber(this.startnum),this.getInformations.unit);
          this.prodprice2 = singlePrice(this,this.getNumber(this.startnum));
        }
      },
      ...
      getInformation() {
        ...
        vue.$http.post(
          ...
          function (vue, data) {
            ...
            vue.getInformations.pagetypeStr = transPackagetype(vue.getInformations.packingList,vue.getInformations.productStore.startnum,vue.getInformations.unit);
            vue.pddes = vue.getInformations.pddes;
            vue.addGoods = data.data.productInfo;
            vue.getInformations.pdpicture = unique(vue.getInformations.pdpicture && vue.getInformations.pdpicture.length > 0 ? vue.getInformations.pdpicture : ["default/imgs/defaultProductImg.jpg"]);
            ...
            vue.minplus= vue.getInformations.productStore.minplus;
            //价格区间list
            ...
            vue.prodprice2 = singlePrice(vue,vue.startnum);//显示的价格
          },
          function (error, data) {
            vue.$message({
              showClose: true,
      ...
      //数量减少
      handleReduce() {
        if((this.selected==="千支"||this.selected==="千")&&this.minplus>0){
          if (this.checkNumber(this.startnum) >= this.getInformations.productStore.startnum ) {
            let nownum1 = this.startnum;
            nownum1=(Number(nownum1 )-Number(this.minplus)).toFixed(3);
            this.startnum =nownum1;
            this.inputFunc(nownum1);
            this.inputBlur(this.startnum);
...
//      数量增加
      handleAdd() {
        if((this.selected==="千支"||this.selected==="千")&&this.minplus>0){
          if (this.checkNumber(this.startnum)>=this.getInformations.productStore.startnum) {
            let nownum2 = this.startnum;
            nownum2=(Number(nownum2)+Number(this.minplus)).toFixed(3);
            this.startnum = nownum2;
            this.inputFunc(nownum2);
            this.inputBlur(this.startnum);
            // this.updatePrice(this.prodprice, this.getNumber(this.startnum));
          }
        }else{
...
//           console.log(this.$store.state.buyGoodsInfo);
          this.$router.push({name: "ShopCar-PlaceOrder"})
//          this.isBuyShow = true;
        }
...
//       this.getInformation();
      let vue = this;
      let param = {pids:this.getInformations.productStore.pdid};
      vue.$http.post(
        ...
        function (vue,data) {
          ...
          if(priceArr){
//              过滤掉价格为0的远期
            vue.getInformations.prices=[];
            for(let j=0;j<data.data.list[0].prices.length;j++){
                if((data.data.list[0].prices)[j].price>0){
                  vue.getInformations.prices.push(data.data.list[0].prices[j])
                }
            }
            vue.prodprice2 = priceArr[0].prices[0].price;
            vue.prodprice = priceArr[0].prices[0].price;
            vue.selecte2d=priceArr[0].prices[0];
            vue.getInformations.showprice = true
            } else {
            vue.getInformations.showprice = false
            }
        },
        function (error,data) {
...
//          {text: "90天后发货", value: '90天后发货'},
      ...
      let ret = await http.post4s(
        data, '/rest/front/product/getProductInfo', {
          id: id,
          productStore: [
            {marketprice: ''}
          ]
      ...
      if(ret.status===200 && ret.data.result===1){
        ...
        data.getInformations = ret.data.data.productInfo;
        data.titleInfo = ret.data.data.productInfo.productname + '_' + ret.data.data.productInfo.material + ' ' + ret.data.data.productInfo.stand + ' ' + ret.data.data.productInfo.level3 + '-紧商网';
        data.keywordsInfo = ret.data.data.productInfo.productname;
        data.descriptionInfo = '紧商网提供' + ret.data.data.productInfo.productname + '：' + ret.data.data.productInfo.material + ' ' + ret.data.data.productInfo.stand + ' ' + ret.data.data.productInfo.level3 + ' ' + ret.data.data.productInfo.productname
        if (data.getInformations.pdstate !== 4) {
          data.canBuy = true;
          // data.$message({
          //   message: '该商品已下架',
          //   type: 'error',
        ...
        data.pddes = data.getInformations.pddes;
        data.addGoods = ret.data.data.productInfo;
        data.getInformations.pdpicture = unique(data.getInformations.pdpicture && data.getInformations.pdpicture.length > 0 ? data.getInformations.pdpicture : ["default/imgs/defaultProductImg.jpg"]);
        data.showPic = data.getInformations.pdpicture[0];
        data.prices = ret.data.data.productInfo.prices;
        data.prodprice = data.prices[0].price;//根据时间变化的价格
        data.threeprice = data.getInformations.productStore.threeprice;
        data.sevenprice = data.getInformations.productStore.sevenprice;
        data.thirtyprice = data.getInformations.productStore.thirtyprice;
        data.sixtyprice = data.getInformations.productStore.sixtyprice;
        data.stopAniu = data.getInformations.productStore.startnum;//判断用，不变的
        data.startnum = data.getInformations.productStore.startnum;//变化的显示的
        data.startnum2 = data.getInformations.productStore.startnum;
        data.startnum3 = data.getInformations.productStore.startnum;
        data.startnum4 = data.getInformations.productStore.startnum;
        //放个加购量
        data.minplus= data.getInformations.productStore.minplus;
        //价格区间list
        data.selecte2d = ret.data.data.productInfo.prices[0];
        data.selected = ret.data.data.productInfo.packingList[0].unit;
        data.selects = ret.data.data.productInfo.packingList;
        data.prodprice2 = singlePrice(data,data.startnum);//显示的价格
      }
      return data;
    }
  }
  ...
  function singlePrice(vue,num) {//计算单价
    num = parseFloat(num);//转换类型做比较
    let rate = 100;
    let range = JSON.parse(vue.getInformations.productStore.intervalprice);
    if (range.length > 0) {
    ...
    let single_price = deleteZero((vue.prodprice * rate / 100).toFixed(5));
