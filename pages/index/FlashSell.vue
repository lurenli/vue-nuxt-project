    ...
    <div class="main_content">
      <div class="top_info">
        <div class="top_info_one">
          <span v-for="(item,index) in listInfo2" :key="index" v-bind:class="{activeinfo:activeNum2==item.value}" @click="searchGoods2(item.value)">{{item.name}}</span>
        </div>
        <div class="top_info_two">
          <li  class="top_info_two_li" v-bind:class="{activeinfo:activeNum===0}" @click="searchGoods(0)">全部</li>
          <li v-for="(item,index) in listInfo" :key="index" class="top_info_two_li" v-bind:class="{activeinfo:activeNum==item.id}" @click="searchGoods(item.id)">{{item.name}}</li>
        </div>
      </div>

      <div class="list_wrap">
        <li v-for="(item,index) in activeGoods" :key="index" @click="jumpDetail(item.pdid,item.id,item.producttype)">
...
<script>
  import Flash from '../../components/IndexRelative/Flash.vue'
  import TimeInfo from '../../components/IndexRelative/SecondMenu/timeInfo.vue'
  export default{
    components:{TimeInfo},
    head() {
      return{
