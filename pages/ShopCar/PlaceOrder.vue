        ...
        <div class="recriver_info">
          ...
          <div class="recriver_info_r right">
            ...
            <div v-if="defaultaddress1==false">
              <div v-for="(infos,i) in addressList" :key="i" class="recriver_info_rc">
        ...
        <div style="border: 1px solid #d9d9d9">
          <div class="recriver_info3_content">
            ...
            <div>
              <label style="margin: 0  0 0 10px;display: inline-block"><input v-model="orderInfo.isbilling" type="radio"
                ...
                style="display: inline-block;margin: 0 10px;vertical-align: top;line-height: 24px">开发票</span></label>
          ...
          <div class="recriver_info_rc3content" v-if="orderInfo.isbilling==1">
            <div class="tick-border" v-if="MRtick==true&&tickList.length>0">
              ...
              <div><span>发票地址&nbsp;:</span>{{tickInfo.address}}</div>
              <div><span>收票地址&nbsp;:</span>{{tickInfo.receiveaddress}}</div>
              <div><span>电话&nbsp;:</span>{{tickInfo.phone}}</div>
            </div>
            <div v-if="MRtick==false">
              <div class="tick-border"  v-for="(item,i) in tickList" :key="i"
                   style="cursor: pointer" @click="changeStyle(item.id)"
                ...
                <div><span>发票地址&nbsp;:</span>{{item.address}}</div>
                <div><span>收票地址&nbsp;:</span>{{item.receiveaddress}}</div>
              ...
              <div v-if="addWrap">
                <div class="tick-add">
                  ...
                  <div><span><span style="color: #f00">*</span>&nbsp;发票地址&nbsp;:</span><input type="text" class="ipt_pay"
                                                                                            v-model="addTickInfo.address">
                  </div>
                  <div><span><span style="color: #f00">*</span>&nbsp;收票地址&nbsp;:</span><input type="text" class="ipt_pay"
                                                                                            v-model="addTickInfo.receiveaddress">
                  </div>
          ...
          <table class="keywords_content_tbd2" style="width: 100%; border-color: #d9d9d9">
            <thead>
            <th></th>
            <th v-for="(aa,i) in theads" :key="i">{{aa}}</th>
            </thead>
            <tbody>
            <template v-for="(item,index) in goodsList" v-if="item">
              ...
              <tr :key="index">
                <td colspan="9" style="padding: 0">
                  ...
                  <div class="logistics_info" v-if="false">
            ...
            <tr>
              <td colspan="10">
                <div class="goback left" @click="jump('ShopCar')"><img src="../../assets/icon/f_back.png"/>返回购物车</div>
...
//              }
                vue.$router.push({name: 'ShopCar-Payment'});
              },
              function (error, data) {
                vue.commitBtnState = false;
                vue.$message({
...
//              console.log(data,1);
      ...
      //添加新的开票信息
      addTick() {
        let successInfo = this.addTickInfo.invoiceheadup !== '' && this.addTickInfo.texno !== '' && this.addTickInfo.bankofaccounts !== '' && this.addTickInfo.account !== '' && this.addTickInfo.linkman !== '' && this.addTickInfo.address !== ''&&this.addTickInfo.receiveaddress!==''&& this.addTickInfo.phone !== '';
        if (successInfo === true) {
          let vue = this;
...
//            console.log(data);
    ...
    mounted() {
      if (this.$store.state.tijiao === '已提交') {
        ...
        this.$router.push({name: 'index-IndustrialProducts'});
      }
      this.getTickList();
      this.getDefaultTick();
